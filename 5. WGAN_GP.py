...
fig.add_trace(go.Scatter(y=price_df[f"pred_price_{epoch-1}"],
                         x=price_df.index, name='Predicted prices from last epoch'), secondary_y=False)
fig.add_trace(go.Scatter(y=price_df[f"pred_price_0"],
                         x=price_df.index, name='Predicted prices from first epoch'), secondary_y=False)
...
fig.update_layout(
    ...
    title=go.layout.Title(
        text=f"WGAN_GP Model Predicted Prices vs. Real Prices<br><sup></sup>",
        xref="paper",
        x=0
    ))
fig.write_image("images/wgan_gp_prices.png")
fig.show()
...
fig.update_layout(
    legend=dict(orientation="h", y=1.2),
    title=go.layout.Title(
        text=f"WGAN_GP Model Error vs. Epochs<br><sup></sup>",
        xref="paper",
        x=0
    ))
fig.write_image("images/wgan_gp_rmse.png")
fig.show()